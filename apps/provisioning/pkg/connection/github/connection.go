...
type GithubFactory interface {
	New(ctx context.Context, ghToken common.RawSecureValue, serverURL string) (Client, error)
}
...
func (c *Connection) Test(ctx context.Context) (*provisioning.TestResults, error) {
	...
	permissionErrors := validatePermissions(permissionTargetApp, c.config.AppID, app.Permissions, c.config.InstallationURL)
	...
	installationPermErrors := validatePermissions(permissionTargetInstallation, c.config.InstallationID, installation.Permissions, c.config.InstallationURL)
...
func validatePermissions(target permissionTarget, id string, permissions Permissions, installationURL string) []provisioning.ErrorDetails {
